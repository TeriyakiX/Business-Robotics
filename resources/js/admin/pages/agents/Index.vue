<template>
    <div class="br-admin-crud">
        ...
        <div class="br-admin-filters-bar">
            <div class="br-admin-filters">
                <div class="br-admin-filter-group">
                    ...
                    <input type="text" v-model="filters.search" placeholder="Поиск AI-агентов..." @input="debouncedFetch"/>
                </div>
                <div class="br-admin-filter-group">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
        ...
        <div v-else-if="items.length === 0" class="br-admin-empty">
            ...
            <h3>Нет AI-агентов</h3>
            <p>Добавьте первого агента, нажав кнопку "Добавить AI-агента"</p>
        </div>
        ...
        <div v-else class="br-admin-items-grid">
            <div v-for="item in items" :key="item.id" class="br-admin-item-card">
                <div class="br-admin-card-header">
                    <div>
                        <h3>{{ item.name }}</h3>
                        <span class="br-admin-tag" v-if="item.tag">{{ item.tag }}</span>
                    </div>
                ...
                <div class="br-admin-card-body">
                    ...
                    <div class="br-admin-features" v-if="item.features && item.features.length">
                        <div class="br-admin-feature" v-for="(feature, idx) in item.features.slice(0, 3)" :key="idx">
                            <span class="br-admin-feature-dot"></span>
                            <span>{{ feature }}</span>
                        </div>
                        <div v-if="item.features.length > 3" class="br-admin-feature-more">
                            + ещё {{ item.features.length - 3 }}
                        </div>
                    </div>
                </div>
        ...
        <div v-if="modalOpen" class="br-admin-modal" @click.self="closeModal">
            <div class="br-admin-modal-container">
                <div class="br-admin-modal-header">
                    <h2>{{ isEdit ? 'Редактирование агента' : 'Новый AI-агент' }}</h2>
                ...
                <form @submit.prevent="submitForm" class="br-admin-modal-form">
                    <div class="br-admin-form-group">
                        ...
                        <input type="text" v-model="form.name" placeholder="Например: AI-LeadGen" required/>
                    </div>

                    <div class="br-admin-form-group">
                        <label>Тег / тип</label>
                        <input type="text" v-model="form.tag" placeholder="Например: Генерация лидов"/>
                    </div>

                    <div class="br-admin-form-group">
                        <label>Описание</label>
                        <textarea v-model="form.description" placeholder="Описание AI-агента" rows="4"></textarea>
                    </div>

                    <div class="br-admin-form-group">
                        <label>Особенности (каждая с новой строки)</label>
                        <textarea v-model="featuresText" placeholder="До 10 000 звонков в сутки&#10;Интеграция с любой CRM&#10;Распознавание возражений" rows="4"></textarea>
                    </div>
...
const submitForm = async () => {
    ...
    try {
        ...
        console.log('Submitting:', { isEdit: isEdit.value, currentId: currentId.value, data });

        if (isEdit.value && currentId.value) {
            await agentsAPI.update(currentId.value, data);
        } else {
...
const deleteItem = async (item) => {
    if (!confirm(`Удалить AI-агента "${item.name}"?`)) return;
    try {
        await agentsAPI.delete(item.id);
        await fetchItems();
    } catch (error) {
