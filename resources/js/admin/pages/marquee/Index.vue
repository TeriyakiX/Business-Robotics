<template>
    <div class="br-admin-crud">
        <div class="br-admin-filters-bar">
            <div class="br-admin-filters">
                <div class="br-admin-filter-group">
                    ...
                    <input type="text" v-model="filters.search" placeholder="Поиск элементов..." @input="debouncedFetch"/>
                </div>
                <div class="br-admin-filter-group">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
        ...
        <div v-if="loading" class="br-admin-loading">
            ...
            <span>Загрузка элементов бегущей строки...</span>
        </div>
        ...
        <div v-else-if="items.length === 0" class="br-admin-empty">
            ...
            <h3>Нет элементов</h3>
            <p>Добавьте первый элемент бегущей строки, нажав кнопку "Добавить элемент"</p>
        </div>
        ...
        <div v-else class="br-admin-items-grid">
            <div v-for="item in items" :key="item.id" class="br-admin-item-card">
                ...
                <div class="br-admin-card-body">
                    <div class="br-admin-marquee-preview">
                        <span class="br-admin-marquee-dot"></span>
                        {{ item.name }}
                    </div>
        ...
        <div v-if="modalOpen" class="br-admin-modal" @click.self="closeModal">
            <div class="br-admin-modal-container">
                <div class="br-admin-modal-header">
                    <h2>{{ isEdit ? 'Редактирование элемента' : 'Новый элемент' }}</h2>
                ...
                <form @submit.prevent="submitForm" class="br-admin-modal-form">
                    <div class="br-admin-form-group">
                        ...
                        <input type="text" v-model="form.name" placeholder="Например: Битрикс24" required/>
                    </div>
...
import { marqueeAPI } from '../../services/api';
...
const normalizeData = (data) => {
    ...
    if (normalized.sort_order !== undefined && normalized.sort_order !== null && normalized.sort_order !== '') {
...
const fetchItems = async () => {
    ...
    try {
        ...
        if (filters.is_active) params.is_active = filters.is_active === 'true'; // Преобразуем в boolean для GET параметра

        console.log('Fetch params:', params);
        const response = await marqueeAPI.getAll(params);
...
const submitForm = async () => {
    ...
    try {
        ...
        if (isEdit.value && currentId.value) {
            await marqueeAPI.update(currentId.value, data);
        } else {
            await marqueeAPI.create(data);
        }
        closeModal();
        await fetchItems();
    } catch (error) {
...
const deleteItem = async (item) => {
    if (!confirm(`Удалить элемент "${item.name}"?`)) return;
    try {
        await marqueeAPI.delete(item.id);
        await fetchItems();
    } catch (error) {
