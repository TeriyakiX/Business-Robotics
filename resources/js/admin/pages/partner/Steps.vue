<template>
    <div class="br-admin-crud">
        <div class="br-admin-filters-bar">
            <div class="br-admin-filters">
                <div class="br-admin-filter-group">
                    ...
                    <input type="text" v-model="filters.search" placeholder="Поиск шагов..." @input="debouncedFetch"/>
                </div>
        ...
        <div v-if="loading" class="br-admin-loading">
            ...
            <span>Загрузка шагов партнерской программы...</span>
        </div>
        ...
        <div v-else-if="items.length === 0" class="br-admin-empty">
            ...
            <h3>Нет шагов</h3>
            <p>Добавьте первый шаг партнерской программы, нажав кнопку "Добавить шаг"</p>
        </div>
        ...
        <div v-else class="br-admin-items-grid">
            <div v-for="item in items" :key="item.id" class="br-admin-item-card">
                <div class="br-admin-card-header">
                    <div>
                        <h3>Шаг {{ item.number }}: {{ item.title }}</h3>
                        <div class="br-admin-meta-tags">
                            <span class="br-admin-step-tag">
        ...
        <div v-if="modalOpen" class="br-admin-modal" @click.self="closeModal">
            <div class="br-admin-modal-container">
                <div class="br-admin-modal-header">
                    <h2>{{ isEdit ? 'Редактирование шага' : 'Новый шаг' }}</h2>
                ...
                <form @submit.prevent="submitForm" class="br-admin-modal-form">
                    <div class="br-admin-form-row">
                        ...
                        <div class="br-admin-form-group">
                            ...
                            <input type="text" v-model="form.title" placeholder="Название шага" required/>
                        </div>
                    </div>

                    <div class="br-admin-form-group">
                        <label>Описание</label>
                        <textarea v-model="form.description" placeholder="Описание шага" rows="3"></textarea>
                    </div>
...
import { partnerStepsAPI } from '../../services/api';
...
const fetchItems = async () => {
    ...
    try {
        ...
        const response = await partnerStepsAPI.getAll(params);
...
const submitForm = async () => {
    ...
    try {
        ...
        if (isEdit.value && currentId.value) {
            await partnerStepsAPI.update(currentId.value, data);
        } else {
            await partnerStepsAPI.create(data);
        }
        closeModal();
        await fetchItems();
    } catch (error) {
...
const deleteItem = async (item) => {
    if (!confirm(`Удалить шаг "${item.title}"?`)) return;
    try {
        await partnerStepsAPI.delete(item.id);
        await fetchItems();
    } catch (error) {
